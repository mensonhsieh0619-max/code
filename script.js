...
function submitHealthData(event) {
    ...
    addNotification(activeUserId(), "健康資料已新增", `BMI ${record.bmi}，FHIR JSON 與 AI 分析已更新。`);
    saveState();
    showToast("健康資料已成功儲存");
    event.target.reset();
...
function renderAIAnalysis() {
    ...
    setHTML("ai-health-panel", `
        <div class="ai-analysis-layout">
            <div class="ai-score-card" style="--score-color:${scoreColor}">
                <span>AI 健康分數</span>
                <strong>${escapeHTML(analysis.score)}</strong>
                <small>${escapeHTML(analysis.riskLevel)}</small>
            </div>
            <div class="ai-trend-summary">
                ...
                <div class="ai-trend-grid">
                    ...
                    ${healthDetailItem("平均 BMI", analysis.trendStats.averageBMI)}
                    ${healthDetailItem("平均心率", `${analysis.trendStats.averageHeartRate} bpm`)}
                    ${healthDetailItem("平均步數", analysis.trendStats.averageSteps)}
                    ${healthDetailItem("總運動時間", `${analysis.trendStats.totalExerciseMinutes} min`)}
                    ${healthDetailItem("睡眠平均", `${analysis.trendStats.averageSleep} h`)}
                    ${healthDetailItem("異常次數", `${analysis.trendStats.abnormalCount} 次`)}
                </div>
            </div>
        </div>
        <div class="ai-risk-chip-list">${analysis.riskTags.map((tag) => `<span class="ai-risk-chip">${escapeHTML(tag)}</span>`).join("") || `<span class="ai-risk-chip">目前無明顯風險</span>`}</div>
        <div class="ai-advice-grid">
            ${adviceCard("飲食建議", analysis.dietAdvice)}
            ${adviceCard("運動建議", analysis.exerciseAdvice)}
            ${adviceCard("睡眠建議", analysis.sleepAdvice)}
            ${adviceCard("就醫提醒", analysis.medicalAdvice)}
            ${adviceCard("生活習慣建議", analysis.lifestyleAdvice)}
        </div>`);
}
...
function runAIAnalysis(accountId = activeUserId()) {
    ...
    if (!latest) {
        return {
            ...
            risk: "尚無資料",
            riskTags: [],
            summary: "請先新增健康資料，系統會根據最近 7 筆紀錄產生 AI 趨勢摘要。",
            healthAdvice: "請先新增健康資料。",
            dietAdvice: "尚無飲食建議。",
            exerciseAdvice: "尚無運動建議。",
            sleepAdvice: "尚無睡眠建議。",
            medicalAdvice: "尚無就醫提醒。",
            lifestyleAdvice: "尚無生活習慣建議。",
            weeklyExercise: 0,
            trendStats: emptyTrendStats()
        };
    }
    const analysis = analyzeRecord(latest, records);
    const trendStats = calculateTrendStats(recent);
    const summary = `最近 ${recent.length} 筆資料平均血壓 ${trendStats.averageBloodPressure}、平均 BMI ${trendStats.averageBMI}、平均心率 ${trendStats.averageHeartRate} bpm，總運動 ${trendStats.totalExerciseMinutes} 分鐘，異常 ${trendStats.abnormalCount} 次。`;
    return {
        ...
        dietAdvice: buildDietAdvice(latest, analysis.riskTags),
        exerciseAdvice: analysis.riskTags.includes("運動不足") || analysis.riskTags.includes("活動量不足") ? "本週運動量未達 150 分鐘或今日步數偏低，建議分散安排快走、慢跑或阻力訓練。" : "運動量維持良好，建議保留每週至少 150 分鐘中等強度活動。",
        sleepAdvice: latest.sleep < 6 ? "睡眠不足，建議固定就寢時間並減少睡前螢幕刺激。" : latest.sleep > 9 ? "睡眠偏長，若伴隨疲倦或精神不佳，建議觀察作息與身體狀況。" : "睡眠時數在合理範圍，建議持續維持規律作息。",
        medicalAdvice: analysis.riskTags.includes("高風險血壓") ? "血壓已達高風險門檻，建議儘快諮詢醫療專業人員並持續量測。" : analysis.riskLevel === "高風險" ? "目前有多項異常指標，若症狀持續或不適，建議就醫評估。" : "目前未達立即就醫警示，仍建議定期追蹤血壓、心率與體重。",
        lifestyleAdvice: latest.stress >= 4 ? "壓力偏高，建議安排放鬆練習、散步或短暫休息，並觀察壓力來源。" : "生活習慣狀態尚可，建議維持飲水、規律運動與固定睡眠。",
        weeklyExercise: recent.reduce((sum, item) => sum + Number(item.exercise || 0), 0),
        trendStats
    };
}
...
function runAIAnalysis(accountId = activeUserId()) {
    ...
    if (latest.diet && latest.diet !== "正常" && latest.diet !== "甇?虜") addRisk(`飲食${latest.diet}`, 5);
    ...
    const abnormalCount = recent.filter((record) => {
        ...
        return rbmi < 18.5 || rbmi >= 24 || record.systolic >= 130 || record.diastolic >= 80 || record.heartRate > 100 || record.heartRate < 50 || record.steps < 6000 || record.sleep < 6 || record.sleep > 9 || record.stress >= 4 || (record.diet && record.diet !== "正常" && record.diet !== "甇?虜");
    }).length;
    const trendStats = {
        averageBloodPressure: `${avg((r) => r.systolic).toFixed(1)}/${avg((r) => r.diastolic).toFixed(1)} mmHg`,
        averageBMI: avg((r) => r.bmi).toFixed(1),
...
function healthStatus(value, normal, warning) {
    if (!Number.isFinite(Number(value))) return { label: "--", className: "muted" };
    if (normal(Number(value))) return { label: "正常", className: "good" };
    if (warning && warning(Number(value))) return { label: "注意", className: "warning" };
...
function ensureHealthFeatureUI() {
    const section = document.getElementById("health-input-section");
    if (!section || section.dataset.healthEnhancedFinal === "true") return;
    section.dataset.healthEnhancedFinal = "true";
    section.innerHTML = `
        <div class="ai-gradient-header">
            <div>
                <span>Health Data</span>
                <h2>健康資料</h2>
                <p>填寫今日量測、活動與生活習慣，送出後會產生健康摘要、AI 分析、FHIR 與趨勢資料。</p>
            </div>
        </div>
        <div class="card form-card health-form-section">
            <form id="health-data-form" onsubmit="submitHealthData(event)">
        ...
        <section id="health-record-list" class="card">
            ...
            <div class="health-record-table-wrapper">
                <table class="health-record-table">
                    <thead><tr><th>日期</th><th>血壓</th><th>體重</th><th>BMI</th><th>心率</th><th>步數</th><th>運動</th><th>睡眠</th><th>狀態</th><th>操作</th></tr></thead>
                    <tbody id="health-record-table-body"></tbody>
                </table>
            </div>
        </section>`;
...
function ensureAIAnalysisMarkup() {
    ...
    panel.innerHTML = `
        <div class="ai-dashboard">
            <div class="ai-gradient-header">
                <div>
                    <span>AI Health Dashboard</span>
                    <h2>AI 健康分析</h2>
                    <p id="ai-summary">目前尚無健康資料，請先新增健康紀錄以產生 AI 分析。</p>
                </div>
                <span id="ai-risk-level" class="ai-status-badge muted">No Data</span>
            </div>
            <div class="ai-dashboard-grid">
                <article class="ai-score-card">
                    <div class="ai-gauge" style="--score:0">
            ...
            <div class="ai-advice-grid">
                <article class="ai-advice-card"><span class="ai-card-icon">${AI_HEALTH_ICONS.diet}</span><h3>飲食</h3><p id="ai-diet-advice">--</p></article>
                <article class="ai-advice-card"><span class="ai-card-icon">${AI_HEALTH_ICONS.exercise}</span><h3>運動</h3><p id="ai-exercise-advice">--</p></article>
                <article class="ai-advice-card"><span class="ai-card-icon">${AI_HEALTH_ICONS.sleep}</span><h3>睡眠</h3><p id="ai-sleep-advice">--</p></article>
                <article class="ai-advice-card"><span class="ai-card-icon">${AI_HEALTH_ICONS.water}</span><h3>飲水</h3><p id="ai-water-advice">--</p></article>
                <article class="ai-advice-card"><span class="ai-card-icon">${AI_HEALTH_ICONS.medical}</span><h3>醫療提醒</h3><p id="ai-medical-advice">--</p></article>
            </div>
            <div class="ai-dashboard-grid bottom">
                <article class="ai-risk-card">
                    <h3>健康風險分析</h3>
                    <div class="radar-wrap"><canvas id="ai-risk-radar" width="420" height="320"></canvas></div>
                </article>
                <article class="ai-trend-summary">
                    <h3>AI 趨勢摘要</h3>
                    <div id="ai-trend-summary" class="ai-trend-grid"></div>
                </article>
            </div>
            <article class="ai-timeline-card">
                <h3>健康建議時間軸</h3>
                <div id="ai-timeline" class="ai-timeline"></div>
            </article>
        </div>`;
    return true;
}
...
function runAIAnalysis(accountId = activeUserId()) {
    ...
    score = Math.max(0, Math.min(100, score));
    const trendStats = calculateTrendStats(recent);
    const bpNormal = latest.systolic < 120 && latest.diastolic < 80;
    const extraExercise = Math.max(0, 150 - weeklyExercise);
    const summary = `近7天平均BMI為${trendStats.averageBMI}。血壓${bpNormal ? "維持正常" : "需要持續觀察"}。本週運動時間${weeklyExercise}分鐘，${extraExercise ? `建議再增加${Math.min(30, extraExercise)}分鐘有氧運動。` : "已達每週150分鐘建議量。"}`;
    ...
    return {
        ...
        trendStats,
        kpis: [
            { icon: AI_HEALTH_ICONS.bmi, label: "BMI", value: latest.bmi, status: bmiStatus },
            { icon: AI_HEALTH_ICONS.bp, label: "血壓", value: `${latest.systolic}/${latest.diastolic}`, status: bpStatus },
            { icon: AI_HEALTH_ICONS.heart, label: "心率", value: `${latest.heartRate} bpm`, status: heartStatus },
            { icon: AI_HEALTH_ICONS.steps, label: "步數", value: Number(latest.steps || 0).toLocaleString(), status: stepsStatus },
            { icon: AI_HEALTH_ICONS.sleep, label: "睡眠", value: `${latest.sleep} 小時`, status: sleepStatus },
            { icon: AI_HEALTH_ICONS.exercise, label: "運動", value: `${weeklyExercise} 分鐘`, status: exerciseStatus }
        ],
...
function calculateTrendStats(records) {
    ...
    return {
        ...
        averageSleep: avg((r) => r.sleep).toFixed(1),
        abnormalCount,
        summary: `最近${records.length}筆資料：平均BMI ${avg((r) => r.bmi).toFixed(1)}、平均血壓 ${avg((r) => r.systolic).toFixed(1)}/${avg((r) => r.diastolic).toFixed(1)} mmHg、平均心率 ${avg((r) => r.heartRate).toFixed(1)} bpm、平均步數 ${Math.round(avg((r) => r.steps)).toLocaleString()}、平均睡眠 ${avg((r) => r.sleep).toFixed(1)} 小時、運動總時間 ${records.reduce((sum, item) => sum + Number(item.exercise || 0), 0)} 分鐘、異常次數 ${abnormalCount} 次。`
    };
}
...
function buildRadarScores(record, weeklyExercise) {
    const bmiScore = Math.max(0, 100 - Math.abs(Number(record.bmi) - 22) * 12);
    const bpScore = Math.max(0, 100 - Math.max(0, record.systolic - 120) * 1.6 - Math.max(0, record.diastolic - 80) * 2);
    const heartScore = Math.max(0, 100 - Math.abs(record.heartRate - 72) * 1.5);
    const exerciseScore = Math.min(100, (weeklyExercise / 150) * 100);
    const sleepScore = Math.max(0, 100 - Math.abs(record.sleep - 7.5) * 18);
    const stressScore = Math.max(0, 120 - Number(record.stress || 3) * 20);
    return [bmiScore, bpScore, heartScore, exerciseScore, sleepScore, stressScore].map((v) => Math.round(Math.max(0, Math.min(100, v))));
}
...
function renderAIAnalysis() {
    ...
    const scoreNode = document.getElementById("ai-score-value");
    document.querySelector(".ai-gauge")?.style.setProperty("--score", scoreValue);
    countUp(scoreNode, scoreValue, analysis.score === "--" ? "--" : null);
    document.getElementById("ai-risk-level").textContent = level.label;
    document.getElementById("ai-risk-level").className = `ai-status-badge ${level.className}`;
    document.getElementById("ai-summary").textContent = analysis.summary;
    document.getElementById("ai-diet-advice").textContent = analysis.dietAdvice;
    document.getElementById("ai-exercise-advice").textContent = analysis.exerciseAdvice;
    document.getElementById("ai-sleep-advice").textContent = analysis.sleepAdvice;
    document.getElementById("ai-water-advice").textContent = analysis.waterAdvice;
    document.getElementById("ai-medical-advice").textContent = analysis.medicalAdvice;
    document.getElementById("ai-risk-tags").innerHTML = analysis.riskTags.map((tag) => `<span class="ai-chip">${escapeHTML(tag)}</span>`).join("") || `<span class="ai-chip good">目前無明顯風險</span>`;
    document.getElementById("ai-kpi-grid").innerHTML = analysis.kpis.map((item) => aiKpiCard(item)).join("");
...
function countUp(node, target, fallback) {
    if (!node) return;
    if (fallback !== null) { node.textContent = fallback; return; }
    ...
    const tick = (now) => {
        ...
        node.textContent = Math.round(start + (end - start) * progress);
...
function drawRadarChart(canvasId, labels, values) {
    ...
    labels.forEach((label, i) => {
        ...
        ctx.fillText(label, cx + Math.cos(angle) * (radius + 42), cy + Math.sin(angle) * (radius + 28));
    });
    ctx.beginPath();
    ...
    modal.innerHTML = `
        <div class="registration-modal-panel health-summary-card">
            ...
            <div class="summary-list">
                ...
                ${summaryRow("血壓", `${record.systolic} / ${record.diastolic}`, bp.label, bp.className)}
                ${summaryRow("心率", `${record.heartRate} bpm`, heart.label, heart.className)}
                ${summaryRow("今日步數", Number(record.steps || 0).toLocaleString(), `達成${stepsPercent}%`, stepsPercent >= 80 ? "good" : "warning")}
                ${summaryRow("運動", record.exerciseType, `${record.exercise}分鐘`, "primary")}
                ${summaryRow("睡眠", `${record.sleep} 小時`, "", "primary")}
            </div>
            <p class="summary-ai-done">AI 已完成分析：${escapeHTML(analysis.riskLevel)}</p>
            <div class="summary-actions">
                <button type="button" class="primary-button" onclick="closeHealthSummaryModal(); showSection('ai-health-section')">查看 AI 報告</button>
                <button type="button" class="secondary-button" onclick="closeHealthSummaryModal(); showSection('fhir-viewer-section')">查看 FHIR</button>
            </div>
        </div>`;
    modal.classList.add("show");
}
...
// PWA registration, install prompt, and network status notices.
...
function initPWAInstallPrompt() {
    ...
    installButton?.addEventListener("click", async () => {
        if (!deferredPWAInstallPrompt) {
            ...
            showPWAToast("請使用 Chrome 或 Edge 的加入主畫面功能");
            return;
        }
    ...
    laterButton?.addEventListener("click", () => {
        document.getElementById("pwa-install-card")?.classList.add("pwa-install-dismissed");
        showPWAToast("稍後仍可從瀏覽器選單加入主畫面");
    });
}

function initNetworkStatusToast() {
    if (window.__aiHealthNetworkStatusToast) return;
    window.__aiHealthNetworkStatusToast = true;

    window.addEventListener("offline", () => {
        showPWAToast("目前為離線模式，仍可查看已快取資料。");
    });
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    initNetworkStatusToast();
});

// Wearable device import simulation.
...
function ensureAuditLogs() {
    ...
    if (!state.auditLogs.length) {
        state.auditLogs.push({ id: "LOG-20260628-001", actor: "王小明", role: "user", action: "新增健康資料", target: "Observation", time: "2026-06-28 10:30", status: "Success" });
        localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
    }
}
...
function renderSecurityCenter() {
    ...
    section.innerHTML = `
        <div class="section-heading"><h2>資料安全與隱私保護中心</h2><p>展示本平台如何透過登入驗證、角色權限、資料授權、敏感資料遮罩、Hash 驗證與區塊鏈授權紀錄保護健康資料。</p></div>
        ...
        <article class="hash-tool-card"><h3>Hash 驗證工具</h3><div class="form-group"><label for="security-hash-input">輸入驗證文字</label><textarea id="security-hash-input" placeholder="輸入要產生 Hash 的文字"></textarea></div><button type="button" class="primary-button" onclick="generateHashFromInput()">產生 Hash</button><pre class="encrypted-box" id="security-hash-output">尚未產生 Hash</pre></article>
        <article class="privacy-note"><h3>健康資料不上鏈，只將授權紀錄 Hash 上鏈</h3><p>平台僅保存授權紀錄 Hash 作為不可竄改存證，不把原始健康資料寫入鏈上。</p><ul><li>避免個資外洩</li><li>保持資料可撤回</li><li>區塊鏈只做不可竄改存證</li></ul></article>
        <div class="card" style="margin-top:18px"><h3>操作紀錄 Audit Log</h3><div class="health-record-table-wrapper"><table class="audit-log-table"><thead><tr><th>時間</th><th>操作者</th><th>角色</th><th>動作</th><th>目標</th><th>狀態</th></tr></thead><tbody>${renderAuditRows()}</tbody></table></div></div>`;
}
...
function simulateEncrypt() {
    ...
    state.securityEncryptedData = `SIM-AES-LIKE:${encodeSecurityPayload(plain)}`;
...
function simulateDecrypt() {
    ...
    if (!encrypted) { showToast("尚無可解密資料"); return; }
    try {
...
async function generateHashFromInput() {
    ...
    if (!input.trim()) { showToast("請輸入要產生 Hash 的文字"); return; }
    let result;
...
loginAccount = async function securityLoginAccount(event) {
    ...
    if (isLoggedIn()) { addAuditLog("登入成功", "Auth", "Success"); saveState(); }
};
...
submitHealthData = function securitySubmitHealthData(event) {
    ...
    if ((state.healthRecords?.length || 0) > before) { addAuditLog("新增健康資料", "Observation", "Success"); saveState(); }
};
...
generateQRCode = function securityGenerateQRCode(event) {
    ...
    if ((state.authorizations?.length || 0) > before) { addAuditLog("產生授權 QR Code", "Authorization", "Success"); saveState(); }
};
...
deleteHealthRecord = function securityDeleteHealthRecord(id) {
    ...
    if ((state.healthRecords?.length || 0) < before) { addAuditLog("刪除健康資料", `Observation ${id}`, "Success"); saveState(); }
};
...
if (typeof importWearableData === "function") {
    ...
    importWearableData = function securityImportWearableData(source) {
        ...
        if ((state.healthRecords?.length || 0) > before) { addAuditLog("匯入穿戴裝置資料", source, "Success"); saveState(); }
    };
}
...
function validateFHIRBundle() {
    ...
    const patientErrors = [];
    if (!patient) {
        patientErrors.push("Patient resource is required");
    } else {
    ...
    const extraObservations = observations.filter((resource) => {
        const code = resource?.code?.coding?.[0]?.code;
        return !observationLabels.some((target) => target.match === code);
    });
    extraObservations.forEach((resource) => {
        const display = resource?.code?.coding?.[0]?.display || "Observation";
        const missing = [];
        if (resource.status !== "final") missing.push("status must be final");
        if (!resource.code) missing.push("code is required");
        if (!resource.subject?.reference) missing.push("subject.reference is required");
        if (!resource.effectiveDateTime) missing.push("effectiveDateTime is required");
        if (!resource.valueQuantity && !resource.component) missing.push("valueQuantity or component is required");
        addFhirCheck(results, `${display} Observation`, missing.length ? "warning" : "pass", missing.join(", ") || "Additional Observation is valid");
    });
...
function renderFHIRValidation() {
    ...
    section.innerHTML = `
        <div class="section-heading fhir-validator-heading">
            <div>
                <h2>FHIR 驗證中心</h2>
                <p>使用目前 generateFHIRBundle() 產生的 FHIR Bundle，展示前端驗證、JSON 預覽與 HAPI FHIR Server 串接流程。</p>
            </div>
        ...
        <div class="fhir-validator-grid">
            <article class="card fhir-validator-card">
                <div class="fhir-card-title-row">
                    <h3>FHIR Validator</h3>
                    <span class="fhir-status-badge ${statusClass}">${results.status}</span>
                </div>
                <div class="fhir-validation-kpis">
                    <div><span>通過數</span><strong>${results.passed}</strong></div>
                    <div><span>警告數</span><strong>${results.warnings}</strong></div>
                    <div><span>錯誤數</span><strong>${results.errors}</strong></div>
                </div>
                <ul class="fhir-validation-list">
                    ${displayItems.map((item) => `
                        <li class="${item.severity}">
                            ${fhirStatusIcon(item.severity)}
                            <span>${escapeHTML(item.label)}：${item.severity === "pass" ? "通過" : item.severity === "warning" ? "警告" : "錯誤"}</span>
            ...
            <article class="card hapi-demo-card">
                <div class="fhir-card-title-row">
                    <h3>HAPI FHIR Server 同步展示</h3>
                    <span class="hapi-mode-badge" id="hapi-mode-badge">${escapeHTML(state.hapiMode || "mock")}</span>
                </div>
                <div class="form-group">
                    <label for="hapi-server-url">Server URL</label>
                    <input id="hapi-server-url" type="url" value="${escapeHTML(state.hapiServerUrl || "https://hapi.fhir.org/baseR4")}" />
                </div>
                <div class="fhir-actions">
                    <button type="button" class="secondary-button" onclick="testHapiConnection()">測試連線</button>
                    <button type="button" class="primary-button" onclick="uploadBundleToHapi()">模擬上傳 Bundle</button>
                    <button type="button" class="secondary-button" onclick="readHapiPatient()">讀取 Patient</button>
                    <button type="button" class="secondary-button" onclick="clearFhirSyncLogs()">清除同步紀錄</button>
                </div>
        ...
        <article class="card fhir-json-card">
            <div class="fhir-card-title-row">
                <h3>FHIR JSON 預覽</h3>
                <div class="fhir-actions">
                    <button type="button" class="secondary-button" onclick="copyFHIRJson()">複製 JSON</button>
                    <button type="button" class="secondary-button" onclick="renderFHIRValidation()">重新驗證</button>
                    <button type="button" class="primary-button" onclick="downloadFHIRJson()">下載 JSON</button>
                </div>
            </div>
            <pre class="code-block fhir-json-preview" id="fhir-validator-json">${escapeHTML(json)}</pre>
        </article>

        <article class="card fhir-log-card">
            <h3>同步紀錄</h3>
            <div class="health-record-table-wrapper">
                <table class="fhir-sync-log-table">
                    <thead><tr><th>同步編號</th><th>Server</th><th>Patient ID</th><th>Observation 數</th><th>模式</th><th>狀態</th><th>時間</th></tr></thead>
                    <tbody id="fhir-sync-log-body"></tbody>
                </table>
            </div>
        </article>`;
    renderHapiSyncSteps();
...
function copyFHIRJson() {
    ...
    if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(text).then(() => showToast("FHIR JSON 已複製")).catch(() => showToast("瀏覽器限制剪貼簿，請手動複製"));
    } else {
...
function downloadFHIRJson() {
    const text = JSON.stringify(fhirBundleForValidation(), null, 2);
...
async function testHapiConnection() {
    const input = document.getElementById("hapi-server-url");
    const server = (input?.value || "https://hapi.fhir.org/baseR4").replace(/\/$/, "");
...
async function simulateHapiUpload() {
    ...
    const steps = [
        ...
        "建立 Patient",
        "建立 Observation",
        "上傳到 HAPI FHIR Server",
        "回傳 Resource ID",
        "同步完成"
    ];
    state.hapiSyncSteps = steps.map((label) => ({ label, status: "pending" }));
    state.hapiStatusText = "FHIR Bundle 同步流程執行中...";
    saveState();
    renderFHIRValidation();

    for (let i = 0; i < steps.length; i += 1) {
        state.hapiSyncSteps = state.hapiSyncSteps.map((step, index) => ({
            ...step,
            status: index < i ? "success" : index === i ? "processing" : "pending"
        }));
        saveState();
        renderHapiSyncSteps();
    ...
    state.fhirSyncLogs.unshift({
        id: `SYNC-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-${String(state.fhirSyncLogs.length + 1).padStart(3, "0")}`,
        server: state.hapiServerUrl || "https://hapi.fhir.org/baseR4",
        bundleId: bundle.id || "bundle-demo-001",
        patientId: patient.id || "patient-demo-001",
        observationCount,
        mode,
...
function renderHapiSyncSteps() {
    ...
    if (!container) return;
    const steps = Array.isArray(state.hapiSyncSteps) && state.hapiSyncSteps.length
        ? state.hapiSyncSteps
        : [
            { label: "驗證 FHIR Bundle", status: "pending" },
            { label: "建立 Patient", status: "pending" },
            { label: "建立 Observation", status: "pending" },
            { label: "上傳到 HAPI FHIR Server", status: "pending" },
            { label: "回傳 Resource ID", status: "pending" },
            { label: "同步完成", status: "pending" }
        ];
    container.innerHTML = steps.map((step, index) => `
        <div class="hapi-step ${escapeHTML(step.status)}">
            <span class="hapi-step-index">${index + 1}</span>
            <span class="hapi-step-label">${escapeHTML(step.label)}</span>
            <span class="hapi-step-status">${escapeHTML(step.status)}</span>
        </div>
    `).join("");
}
...
function renderFhirSyncLogs() {
    ...
    body.innerHTML = state.fhirSyncLogs.map((log) => `
        <tr>
            ...
            <td>${escapeHTML(log.server)}</td>
            <td>${escapeHTML(log.patientId)}</td>
            <td>${escapeHTML(String(log.observationCount))}</td>
            <td><span class="hapi-mode-badge">${escapeHTML(log.mode)}</span></td>
            <td><span class="fhir-status-badge ${String(log.status).toLowerCase() === "success" ? "valid" : "error"}">${escapeHTML(log.status)}</span></td>
            <td>${escapeHTML(log.time)}</td>
        </tr>
    `).join("") || `<tr><td colspan="7" class="empty">尚無同步紀錄</td></tr>`;
}
